...
export default async function CustomerProfilePage({ params }: { params: { id: string } }) {
  ...
  const [{ data: customer }, { data: quotations }, { data: invoices }] = await Promise.all([
    supabase.from("customers").select("*").eq("id", params.id).eq("user_id", user.id).single(),
    supabase.from("quotations").select("*").eq("customer_id", params.id).eq("user_id", user.id).order("date", { ascending: false }),
    supabase.from("invoices").select("*").eq("customer_id", params.id).eq("user_id", user.id).order("date", { ascending: false }),
  ]);
